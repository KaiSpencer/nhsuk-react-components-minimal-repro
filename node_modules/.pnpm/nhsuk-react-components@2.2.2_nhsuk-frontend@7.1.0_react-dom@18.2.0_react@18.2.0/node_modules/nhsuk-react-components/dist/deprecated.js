...
const PanelGroup = (_a) => {
    ...
    React.Children.forEach(children, (child) => {
        if (child && React.isValidElement(child) && child.type === Panel) {
            panelCount += 1;
        }
    });
...
const PromoContext = React.createContext({ isGroup: false, promoCount: 0 });
...
const PromoGroup = (_a) => {
    ...
    let promoCount = 0;
    React.Children.forEach(children, (child) => {
        if (child && React.isValidElement(child) && child.type === Promo) {
            promoCount += 1;
        }
    });
...
const Promo = (props) => {
    const { isGroup, promoCount } = React.useContext(PromoContext);
    let promoWidth;
    if (isGroup) {
        switch (promoCount) {
            case 2:
